...
import (
	...
	"movieexample.com/movie/internal/processor"
...
func main() {
	...
	lockProvider, err := consullock.NewLockProvider(logger, cfg.ServiceDiscovery.Consul.Address)
	if err != nil {
		logger.Fatal("Failed to create lock provider", zap.Error(err))
	}
